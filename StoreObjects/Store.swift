...
class Store: ObservableObject {
    ...
    @Published private var advertisement: ads
            ...
            do {
                ...
                case .nonConsumable:
                    if let car = cars.first(where: { $0.id == transaction.productID }) {
                        ...
                        print ("CARS FOUND: ", purchasedCars)
                    }
                case .nonRenewable:
    ...
    func description (for productId: String) -> String {
    ...
    func advertisementCheck (for productId: String) -> ads {
        
        for i in purchasedSubscriptions{
                
                print ("SUBS:", i)
        }
        if purchasedSubscriptions.isEmpty && purchasedCars.isEmpty {
            return .AdsOn
        }
        else {
            return .AdsOff
